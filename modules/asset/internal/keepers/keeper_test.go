...
import (
	...
	"github.com/SoftWorxDevelopments/mypc-sdk/modules/asset/internal/types"
...
func TestTokenKeeper_TokenReserved(t *testing.T) {
	input := createTestInput()
	addr, _ := sdk.AccAddressFromBech32("mypos133w8vwj73s4h2uynqft9gyyy52cr6rg8dskv3h")
	...
	// issue mypc token success
	err = input.tk.IssueToken(input.ctx, "MYPC token", "mypc", sdk.NewInt(2100), testAddr,
		true, true, false, true, "", "", types.TestIdentityString)
	require.NoError(t, err)

	// mypc owner issue btc token success
	err = input.tk.IssueToken(input.ctx, "BTC token", "btc", sdk.NewInt(2100), testAddr,
		true, true, false, true, "", "", types.TestIdentityString)
	require.NoError(t, err)

	// only mypc owner can issue reserved token
...
func TestTokenKeeper_TransferOwnership(t *testing.T) {
	input := createTestInput()
	symbol := "abc"
	var addr1, _ = sdk.AccAddressFromBech32("mypos133w8vwj73s4h2uynqft9gyyy52cr6rg8dskv3h")
...
func TestTokenKeeper_MintToken(t *testing.T) {
	input := createTestInput()
	symbol := "abc"
	var addr, _ = sdk.AccAddressFromBech32("mypos133w8vwj73s4h2uynqft9gyyy52cr6rg8dskv3h")
