...
func (keeper BaseKeeper) IssueToken(ctx sdk.Context, name string, symbol string, totalSupply sdk.Int, owner sdk.AccAddress,
	...
	var mypcToken types.Token
	// only mypc owner can issue reserved token
	if types.IsReservedSymbol(symbol) && symbol != myposchain.MYPC {
		mypcToken = keeper.GetToken(ctx, myposchain.MYPC)
		if mypcToken == nil || !owner.Equals(mypcToken.GetOwner()) {
			return types.ErrInvalidIssueOwner()
		}
	}

	// only mypc owner can issue .suffix token
...
func (keeper BaseKeeper) DeductIssueFee(ctx sdk.Context, addr sdk.AccAddress, amt int64) sdk.Error {
	return keeper.bkx.DeductInt64MypcFee(ctx, addr, amt)
}
