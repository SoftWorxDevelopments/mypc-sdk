...
func Test_handleMsg(t *testing.T) {
	input := createTestInput()
	h := asset.NewHandler(input.tk)
	owner, _ := sdk.AccAddressFromBech32("mypos133w8vwj73s4h2uynqft9gyyy52cr6rg8dskv3h")

	err := input.tk.AddToken(input.ctx, testAddr, myposchain.NewMypcCoins(1e18))
...
func testIssueTokenDeductFee(t *testing.T, symbol string) {
	...
	require.Equal(t, sdk.NewInt(1e18-types.DefaultParams().GetIssueTokenFee(symbol)), coins.AmountOf("mypc"))
}
