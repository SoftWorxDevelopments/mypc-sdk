...
func (k *mocBankxKeeper) DeductInt64MypcFee(ctx sdk.Context, addr sdk.AccAddress, amt int64) sdk.Error {
	return k.DeductFee(ctx, addr, myposchain.NewMypcCoins(amt))
}
...
func newContextAndKeeper(chainid string) (sdk.Context, *Keeper) {
	...
	keeper := keepers.NewKeeper(key,
		...
		&mocAssetKeeper{
			tokenIssuer: map[string]sdk.AccAddress{"mypc": simpleAddr("00000")},
		},
...
func Test1(t *testing.T) {
	...
	refLog := "Deduct 1000000000mypc from cosmos1qy352eufqy352eufqy352eufqy35qqqrctskts"
	require.Equal(t, refLog, logStr)
	handlerFunc(ctx, types.MsgAliasUpdate{Owner: bob, Alias: "superman", IsAdd: true, AsDefault: false})
	refLog = "Deduct 1000000000mypc from cosmos1qy352eufqy352eufqy352eufqy35qqqz9ayrkz"
	require.Equal(t, refLog, logStr)
	...
	require.Equal(t, types.ErrAliasAlreadyExists().Result(), res)
	aliasMypc := "mypos-usdt.t"
	res = handlerFunc(ctx, types.MsgAliasUpdate{Owner: tom, Alias: aliasMypc, IsAdd: true})
	require.Equal(t, types.ErrCanOnlyBeUsedByMypcOwner(aliasMypc).Result(), res)

	mypcOwner := simpleAddr("00000")
	refLog = "Deduct 1000000000mypc from cosmos1qy352eufqy352eufqy352eufqy35qqqqkc9q90"
	handlerFunc(ctx, types.MsgAliasUpdate{Owner: mypcOwner, Alias: aliasMypc, IsAdd: true, AsDefault: true})
	addr, isDefault := keeper.GetAddressFromAlias(ctx, aliasMypc)
	require.Equal(t, mypcOwner, sdk.AccAddress(addr))
	require.Equal(t, true, isDefault)
	require.Equal(t, refLog, logStr)

	handlerFunc(ctx, types.MsgAliasUpdate{Owner: alice, Alias: "sup", IsAdd: true, AsDefault: false})
	refLog = "Deduct 500000000000mypc from cosmos1qy352eufqy352eufqy352eufqy35qqqrctskts"
	require.Equal(t, refLog, logStr)
	handlerFunc(ctx, types.MsgAliasUpdate{Owner: alice, Alias: "supe", IsAdd: true, AsDefault: false})
	refLog = "Deduct 200000000000mypc from cosmos1qy352eufqy352eufqy352eufqy35qqqrctskts"
	require.Equal(t, refLog, logStr)
	handlerFunc(ctx, types.MsgAliasUpdate{Owner: alice, Alias: "super", IsAdd: true, AsDefault: false})
	refLog = "Deduct 100000000000mypc from cosmos1qy352eufqy352eufqy352eufqy35qqqrctskts"
	require.Equal(t, refLog, logStr)
	...
	refLog = "Deduct 100000000000mypc from cosmos1qy352eufqy352eufqy352eufqy35qqqz9ayrkz"
	require.Equal(t, refLog, logStr)
	handlerFunc(ctx, types.MsgAliasUpdate{Owner: bob, Alias: "123456", IsAdd: true, AsDefault: false})
	refLog = "Deduct 10000000000mypc from cosmos1qy352eufqy352eufqy352eufqy35qqqz9ayrkz"
	require.Equal(t, refLog, logStr)
...
func TestReservedAliases(t *testing.T) {
	...
	for _, alias := range reservedAliases {
		...
		require.Equal(t, types.ErrCanOnlyBeUsedByMypcOwner(alias).Result(), res)
	}
}
