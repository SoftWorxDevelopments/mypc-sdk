...
func (k Keeper) DeductInt64MypcFee(ctx sdk.Context, addr sdk.AccAddress, amt int64) sdk.Error {
	return k.bnk.DeductInt64MypcFee(ctx, addr, amt)
}
...
func (k Keeper) GetMarketVolume(ctx sdk.Context, stock, money string, stockVolume, moneyVolume sdk.Dec) sdk.Dec {
	volume := sdk.ZeroDec()
	if stock == myposchain.MYPC {
		volume = stockVolume
	} else if money == myposchain.MYPC {
		volume = moneyVolume
	} else if marketInfo, err := k.GetMarketInfo(ctx, myposchain.GetSymbol(myposchain.MYPC, money)); err == nil {
		if marketInfo.LastExecutedPrice.IsZero() {
			return volume
		}
		volume = moneyVolume.Quo(marketInfo.LastExecutedPrice)
	} else if marketInfo, err := k.GetMarketInfo(ctx, myposchain.GetSymbol(myposchain.MYPC, stock)); err == nil {
		if marketInfo.LastExecutedPrice.IsZero() {
			return volume
		}
		volume = stockVolume.Quo(marketInfo.LastExecutedPrice)
	} else if marketInfo, err := k.GetMarketInfo(ctx, myposchain.GetSymbol(money, myposchain.MYPC)); err == nil {
		volume = moneyVolume.Mul(marketInfo.LastExecutedPrice)
	} else if marketInfo, err := k.GetMarketInfo(ctx, myposchain.GetSymbol(stock, myposchain.MYPC)); err == nil {
