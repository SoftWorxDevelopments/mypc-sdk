...
func getDelistKeyRangeByTime(time int64) (start, end []byte) {
	start = myposchain.ConcatKeys(DelistKey, int64ToBigEndianBytes(0), []byte{0x0})
	end = myposchain.ConcatKeys(DelistKey, int64ToBigEndianBytes(time), []byte{0x1})
	return
}
