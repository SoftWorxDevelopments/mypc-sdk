...
var unitTestChainID = "mypos-test"
...
func TestUnfreezeCoinsForOrder(t *testing.T) {
	...
	refouts := []string{
		...
		"unfreeze 10 mypc at cosmos1qy352eufqy352eufqy352eufqy35qqqptw34ca",
		"unfreeze 20 mypc at cosmos1qy352eufqy352eufqy352eufqy35qqqptw34ca",
	}
...
func TestRemoveOrders(t *testing.T) {
	...
	mypcKeeper := keepers.NewOrderKeeper(keys.marketKey, "mypc/usdt", msgCdc)
	...
	records := []string{
		...
		"unfreeze 60 mypc at cosmos1qy352eufqy352eufqy352eufqy35qqqyej8x24",
	}
...
func TestDelist(t *testing.T) {
	...
	records := []string{
		...
		"unfreeze 5 mypc at cosmos1qy352eufqy352eufqy352eufqy35qqqyej8x24",
		"send 5 mypc from cosmos1qy352eufqy352eufqy352eufqy35qqqyej8x24 to cosmos1qy352eufqy352eufqy352eufqy35qqqz9ayrkz",
		...
		"unfreeze 25 mypc at cosmos1qy352eufqy352eufqy352eufqy35qqqyej8x24",
		"send 25 mypc from cosmos1qy352eufqy352eufqy352eufqy35qqqyej8x24 to cosmos1qy352eufqy352eufqy352eufqy35qqszrgzaze",
		...
		"unfreeze 25 mypc at cosmos1qy352eufqy352eufqy352eufqy35qqqf464exq",
		"send 25 mypc from cosmos1qy352eufqy352eufqy352eufqy35qqqf464exq to cosmos1qy352eufqy352eufqy352eufqy35qqszrgzaze",
		"unfreeze 27 usdt at cosmos1qy352eufqy352eufqy352eufqy35qqszrgzaze",
...
func TestRemoveExpiredMarket(t *testing.T) {
	...
	delistKeeper.AddDelistRequest(input.ctx, 8, "abc/mypc")
	delistKeeper.AddDelistRequest(input.ctx, 7, "abd/mypc")
	delistKeeper.AddDelistRequest(input.ctx, 6, "abe/mypc")
	delistKeeper.AddDelistRequest(input.ctx, 5, "abf/mypc")
	delistKeeper.AddDelistRequest(input.ctx, 4, "abg/mypc")
	delistKeeper.AddDelistRequest(input.ctx, 3, "abh/mypc")
	delistSymbols := delistKeeper.GetDelistSymbolsBeforeTime(input.ctx, 8)
	require.EqualValues(t, 6, len(delistSymbols))

	orderInfo := Order{
		TradingPair: "abc/mypc",
		TimeInForce: GTE,
		ExistBlocks: 100,
		Sender:      haveMypcAddress,
	}
	orderKeeper := keepers.NewOrderKeeper(input.mk.GetMarketKey(), "abc/mypc", types.ModuleCdc)
	for i := 0; i < 3; i++ {
		tmp := orderInfo
	...
	require.EqualValues(t, "abc/mypc", delistSymbols[len(delistSymbols)-1])
	require.EqualValues(t, "abe/mypc", delistSymbols[2])
	...
	require.EqualValues(t, "abd/mypc", delistSymbols[0])
	require.EqualValues(t, "abc/mypc", delistSymbols[1])
	orders = orderKeeper.GetOlderThan(input.ctx, 100)
	require.EqualValues(t, 3, len(orders))
...
func TestEndBlocker(t *testing.T) {
	...
	orderKeeper := keepers.NewOrderKeeper(input.mk.GetMarketKey(), GetSymbol(stock, myposchain.MYPC), types.ModuleCdc)
...
func TestCalFrozenFeatureFee(t *testing.T) {
	...
	err := sellAccount.SetCoins(sdk.NewCoins(sdk.NewCoin(stock, sdk.NewInt(10000)),
		sdk.NewCoin(myposchain.MYPC, sdk.NewInt(10000))))
	require.Nil(t, err)
	...
	require.EqualValues(t, sellAccount.GetCoins().AmountOf(myposchain.MYPC).Int64(), 12400)
	require.EqualValues(t, buyAccount.GetCoins().AmountOf(myposchain.MYPC).Int64(), 7400)
	...
	require.EqualValues(t, sellAccount.GetCoins().AmountOf(myposchain.MYPC).Int64(), 14700)
	require.EqualValues(t, buyAccount.GetCoins().AmountOf(myposchain.MYPC).Int64(), 4700)
...
func TestChargeFee(t *testing.T) {
	...
	require.EqualValues(t, fmt.Sprintf("send 1 mypc from %s to %s", from.String(), to.String()), keeper.records[0])
	...
	require.EqualValues(t, fmt.Sprintf("send 99 mypc from %s to %s", from.String(), to.String()), keeper.records[0])
	require.EqualValues(t, fmt.Sprintf("addr : %s, fee : %d", from, 9900), keeper.records[1])
	keeper.cleanRecord()

	chargeFee(ctx, 10000, from, keeper)
	require.EqualValues(t, fmt.Sprintf("send 100 mypc from %s to %s", from.String(), to.String()), keeper.records[0])
	require.EqualValues(t, fmt.Sprintf("addr : %s, fee : %d", from, 9900), keeper.records[1])
	keeper.cleanRecord()
