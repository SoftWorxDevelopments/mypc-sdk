...
var currDealRecordInmyposchain int
...
func (order *mocOrder) Deal(otherSide OrderForTrade, amount int64, price sdk.Dec) {
	...
	if currDealRecordInmyposchain >= len(currDealRecordList) {
		fmt.Printf("More deals than expected! %d>=%d", currDealRecordInmyposchain, len(currDealRecordList))
		testHandler.Errorf("More deals than expected!")
	} else {
		dr := currDealRecordList[currDealRecordInmyposchain]
		currDealRecordInmyposchain++
		pass := dr.self == order.GetOwner().String() && dr.other == other.GetOwner().String() &&
			dr.amount == amount && dr.price.Equal(price)
		if !pass {
			testHandler.Errorf("incorrect deal! i:%d this:%s that:%s a:%d p:%s\n", currDealRecordInmyposchain, dr.self,
