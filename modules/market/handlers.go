...
func NewHandler(k keepers.Keeper) sdk.Handler {
	return func(ctx sdk.Context, msg sdk.Msg) sdk.Result {
		...
		default:
			return myposchain.ErrUnknownRequest(ModuleName, msg)
		}
	}
}
...
func checkMsgCreateTradingPair(ctx sdk.Context, msg types.MsgCreateTradingPair, keeper keepers.Keeper) sdk.Error {
	...
	if !keeper.HasCoins(ctx, msg.Creator, myposchain.NewMypcCoins(marketParams.CreateMarketFee)) {
...
func handleFeeForCreateOrder(ctx sdk.Context, keeper keepers.Keeper, amount int64, denom string,
	...
	if frozenFee != 0 {
		if err := keeper.FreezeCoins(ctx, sender, myposchain.NewMypcCoins(frozenFee)); err != nil {
			return err
		}
	}
	if featureFee != 0 {
		if err := keeper.FreezeCoins(ctx, sender, myposchain.NewMypcCoins(featureFee)); err != nil {
			return err
		}
	}
	return nil
...
func checkMsgCreateOrder(ctx sdk.Context, keeper keepers.Keeper, msg types.MsgCreateOrder, mypcFee int64, amount int64, denom string, seq uint64) sdk.Error {
	if mypcFee != 0 {
		if !keeper.HasCoins(ctx, msg.Sender, sdk.Coins{sdk.NewCoin(myposchain.MYPC, sdk.NewInt(mypcFee))}) {
			return types.ErrInsufficientCoins()
		}
	}
	stock, money := SplitSymbol(msg.TradingPair)
	totalAmount := sdk.NewInt(amount)
	if (stock == myposchain.MYPC && msg.Side == types.SELL) ||
		(money == myposchain.MYPC && msg.Side == types.BUY) {
		totalAmount = totalAmount.AddRaw(mypcFee)
	}
