...
import (
	...
	"github.com/SoftWorxDevelopments/mypc-sdk/modules/market/match"
...
func unfreezeCoinsInOrder(ctx sdk.Context, order *types.Order, bxKeeper types.ExpectedBankxKeeper) {
	...
	if err := bxKeeper.UnFreezeCoins(ctx, order.Sender, myposchain.NewCoins(frozenToken, order.Freeze)); err != nil {
		ctx.Logger().Error("%s", err.Error())
	}
}
...
func chargeOrderCommission(ctx sdk.Context, order *types.Order, feeForZeroDeal int64,
	...
	if order.FrozenCommission != 0 {
		if err := bxKeeper.UnFreezeCoins(ctx, order.Sender, myposchain.NewMypcCoins(order.FrozenCommission)); err != nil {
			ctx.Logger().Error("%s", err.Error())
		}
...
func chargeOrderFeatureFee(ctx sdk.Context, order *types.Order, freeTimeBlocks int64,
	...
	if order.TimeInForce == GTE && order.FrozenFeatureFee != 0 {
		if err := bxKeeper.UnFreezeCoins(ctx, order.Sender, myposchain.NewMypcCoins(order.FrozenFeatureFee)); err != nil {
			ctx.Logger().Error("%s", err.Error())
		}
...
func chargeFee(ctx sdk.Context, fee int64, userAddr sdk.AccAddress, keeper types.Keeper) {
	...
	if rebateAmount > 0 {
		if err := keeper.SendCoins(ctx, userAddr, refereeAddr, myposchain.NewMypcCoins(rebateAmount)); err != nil {
			ctx.Logger().Error("%s", err.Error())
		}
	}
