...
import (
	...
	simulationx "github.com/SoftWorxDevelopments/mypc-sdk/simulation"
...
func createMsgCreateTradingPair(r *rand.Rand, ctx sdk.Context, k keepers.Keeper, ask asset.Keeper, fromAddr sdk.AccAddress) (types.MsgCreateTradingPair, error) {
	...
	if money != myposchain.MYPC && stock != myposchain.MYPC {
		if _, err := k.GetMarketInfo(ctx, myposchain.GetSymbol(stock, myposchain.MYPC)); err != nil {
...
func randomTradingPair(r *rand.Rand, ctx sdk.Context, ask asset.Keeper) (stock, money string) {
	...
	stock = tokenList[simulationx.GetRandomElemInmyposchain(r, len(tokenList))].GetSymbol()
	for {
		money = tokenList[simulationx.GetRandomElemInmyposchain(r, len(tokenList))].GetSymbol()
		if stock != money {
			break
		}
	}
