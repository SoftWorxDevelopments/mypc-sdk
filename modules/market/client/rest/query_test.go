...
func TestQuery(t *testing.T) {
	...
	req, _ := http.NewRequest("GET", "http://example.com/market/trading-pairs/etc/mypc", nil)
	...
	req, _ = http.NewRequest("GET", "http://example.com/market/orderbook/etc/mypc", nil)
	router.ServeHTTP(respWr, req)
	...
	req, _ = http.NewRequest("GET", "http://example.com/market/orders/mypos1px8alypku5j84qlwzdpynhn4nyrkagaytu5u4a-1025", nil)
	router.ServeHTTP(respWr, req)
	...
	req, _ = http.NewRequest("GET", "http://example.com/market/orders/account/mypos1px8alypku5j84qlwzdpynhn4nyrkagaytu5u4a", nil)
	router.ServeHTTP(respWr, req)
	assert.Equal(t, "custom/market/user-order-list", ResultPath)
	assert.Equal(t, keepers.QueryUserOrderList{
		User: "mypos1px8alypku5j84qlwzdpynhn4nyrkagaytu5u4a",
	}, ResultParam)
