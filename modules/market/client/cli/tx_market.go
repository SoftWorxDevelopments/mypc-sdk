...
func CreateMarketCmd(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `generate a tx and sign it to create trading pair in myposchain blockchain. 

Example : 
	cetcli tx market create-trading-pair  \
	--from bob --chain-id=myposmyposchain  \
	--stock=eth --money=mypc --order-precision=8 \
	--price-precision=8 --gas 20000 --fees=1000mypc`,
...
Example 
	...
	--time=1000000 --trading-pair=etc/mypc --from=bob --chain-id=myposmyposchain \
	--gas=1000000 --fees=1000mypc`,
		RunE: func(cmd *cobra.Command, args []string) error {
	...
	cmd.Flags().String(FlagSymbol, "btc/mypc", "The market trading-pair")
...
func ModifyTradingPairPricePrecision(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Modify the price precision of the trading pair in the myposchain.

Example: 
	cetcli tx market modify-price-precision --trading-pair=etc/mypc \
	--price-precision=9 --from=bob --chain-id=myposmyposchain \
	--gas=10000000 --fees=10000mypc`,
		RunE: func(cmd *cobra.Command, args []string) error {
