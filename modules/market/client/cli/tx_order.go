...
Example: 
	 cetcli tx market create-ioc-order --trading-pair=btc/mypc \
	--order-type=2 --price=520 --quantity=10000000 \
	--side=1 --price-precision=10 --from=bob --identify=1 \
	--chain-id=myposmyposchain --gas=10000 --fees=1000mypc`,
...
Example:
	cetcli tx market create-gte-order --trading-pair=btc/mypc \
...
Examples:
	...
	--trust-node=true --from=bob --chain-id=myposmyposchain`,
		RunE: func(cmd *cobra.Command, args []string) error {
