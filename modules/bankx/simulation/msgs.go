...
func SimulateMsgSend(mapper auth.AccountKeeper, bk bankx.Keeper) simulation.Operation {
	...
	return func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, accs []simulation.Account) (
		...
		lockCoinsFee := myposchain.NewMypcCoins(bk.GetParams(ctx).LockCoinsFeePerDay)
...
func createMsgSend(r *rand.Rand, ctx sdk.Context, accs []simulation.Account, mapper auth.AccountKeeper, lockCoinsFee sdk.Coins) (
	...
	denomInmyposchain := r.Intn(len(initFromCoins))
	amt, goErr := simulation.RandPositiveInt(r, initFromCoins[denomInmyposchain].Amount)
	if goErr != nil {
		return fromAcc, "skipping bank send due to account having no coins of denomination " + initFromCoins[denomInmyposchain].Denom, msg, false
	}

	coins := sdk.Coins{sdk.NewCoin(initFromCoins[denomInmyposchain].Denom, amt)}
...
func SimulateMsgSetMemoRequired(k bankx.Keeper) simulation.Operation {
	return func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, accounts []simulation.Account) (
		...
		ok := myposchainsim.SimulateHandleMsg(msg, bankx.NewHandler(k), ctx)
