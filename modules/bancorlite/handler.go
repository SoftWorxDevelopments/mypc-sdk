...
func handleMsgBancorInit(ctx sdk.Context, k Keeper, msg types.MsgBancorInit) sdk.Result {
	...
	if err := k.DeductInt64MypcFee(ctx, msg.Owner, fee); err != nil {
		return err.Result()
	}
	var precision byte
...
func handleMsgBancorTrade(ctx sdk.Context, k Keeper, msg types.MsgBancorTrade) sdk.Result {
	...
	if exist {
		if err := k.DeductFee(ctx, msg.Sender, sdk.NewCoins(sdk.NewCoin(myposchain.MYPC, balance))); err != nil {
			return err.Result()
		}
		if err := k.SendCoins(ctx, msg.Sender, rebateAcc, sdk.NewCoins(sdk.NewCoin(myposchain.MYPC, rebate))); err != nil {
			return err.Result()
		}
	} else {
		if err := k.DeductFee(ctx, msg.Sender, sdk.NewCoins(sdk.NewCoin(myposchain.MYPC, commission))); err != nil {
			return err.Result()
		}
	}
