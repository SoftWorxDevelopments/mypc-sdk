...
func SimulateMsgBancorInit(assetKeeper asset.Keeper, blk bancorlite.Keeper) simulation.Operation {
	return func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, accs []simulation.Account) (
		...
		newSymbol := myposchainsim.RandomSymbol(r, symbolPrefix, 3)
		identity := myposchainsim.RandomSymbol(r, "", 30)
		amount := 1e10 + r.Int63n(1e12)

		// issue new token
		issueTokenMsg := createMsgIssueToken(newSymbol, amount, addr, identity)
		issueTokenOK := myposchainsim.SimulateHandleMsg(issueTokenMsg, asset.NewHandler(assetKeeper), ctx)
		if !issueTokenOK {
		...
		bancorInitOk := myposchainsim.SimulateHandleMsg(bancorInitMsg, bancorlite.NewHandler(blk), ctx)
...
func createMsgBancorInit(r *rand.Rand,
	...
	return bancorlite.MsgBancorInit{
		Owner:              owner,
		Stock:              stockSymbol,
		Money:              myposchain.DefaultBondDenom,
...
func simulateMsgBancorSell(blk bancorlite.Keeper,
	...
	ok := myposchainsim.SimulateHandleMsg(msg, bancorlite.NewHandler(blk), ctx)
