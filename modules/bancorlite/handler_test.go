...
import (
	...
	"github.com/SoftWorxDevelopments/mypc-sdk/modules/bancorlite"
	...
	"github.com/SoftWorxDevelopments/mypc-sdk/modules/market"
...
var (
	haveMypcAddress            = getAddr("000001")
	notHaveMypcAddress         = getAddr("000002")
...
func prepareAsset(t *testing.T, tk asset.Keeper, ctx sdk.Context, addrForbid, tokenForbid bool) {
	...
	require.Equal(t, true, ret.IsOK(), "issue mypc should succeed", ret)
}
...
func prepareAccounts(ctx sdk.Context, ak auth.AccountKeeper) {
	...
	_ = onlyIssueToken.SetCoins(myposchain.NewMypcCoins(asset.DefaultIssue3CharTokenFee))
...
func prepareMarket(ctx sdk.Context, keeper market.Keeper) {
	...
	_ = keeper.SetMarket(ctx, market.MarketInfo{Stock: stock, Money: "mypc", LastExecutedPrice: sdk.NewDec(1e4)})
}
...
func Test_handleMsgBancorTradeAfterInit(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want: sdk.ErrInsufficientCoins("insufficient account funds; 203696000000mypc,400000teos,209999858490tusdt < 6300000000000mypc").Result(),
		},
		{
