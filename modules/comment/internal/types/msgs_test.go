...
func Test1(t *testing.T) {
	...
	refs := getRefs()
	////// ShortHanzi
	msg := NewMsgCommentToken(simpleAddr("00003"), "mypc", 1, "First Comment", Text0, ShortHanzi, refs)
	...
	////// ShortHanziLZ4
	msg = NewMsgCommentToken(simpleAddr("00003"), "mypc", 1, "First Comment", Text1, ShortHanzi, refs)
	tc = NewTokenComment(msg, 108, 1000)
	if msg.ContentType != ShortHanziLZ4 || Text1 != tc.Content || tc.ContentType != UTF8Text {
		t.Errorf("Invalid Token Comment!")
	}
	msg = NewMsgCommentToken(simpleAddr("00003"), "mypc", 1, "First Comment", Text2, ShortHanzi, refs)
	tc = NewTokenComment(msg, 108, 1000)
	...
	msg = NewMsgCommentToken(simpleAddr("00003"), "mypc", 1, "First Comment", s, RawBytes, refs)
	tc = NewTokenComment(msg, 108, 1000)
	...
	////// UTF8Text
	s = "孜孜不倦"
	msg = NewMsgCommentToken(simpleAddr("00003"), "mypc", 1, "First Comment", s, UTF8Text, refs)
	tc = NewTokenComment(msg, 108, 1000)
	fmt.Printf("Here! %s %d\n", tc.Content, tc.ContentType)
	...
	////// HTTP
	s = "http://google.com"
	msg = NewMsgCommentToken(simpleAddr("00003"), "mypc", 1, "First Comment", s, HTTP, refs)
	tc = NewTokenComment(msg, 108, 1000)
	fmt.Printf("Here! %s %d\n", tc.Content, tc.ContentType)
	...
	msg = NewMsgCommentToken(simpleAddr("00003"), "mypc", 1, "", s, HTTP, []CommentRef{refs[0]})
	if res := msg.ValidateBasic(); res == nil {
		t.Errorf("This should be an invalid Msg!")
	} else {
		logStrList = append(logStrList, res.ABCILog())
	...
	text := Text3 + Text3
	msg = NewMsgCommentToken(simpleAddr("00003"), "mypc", 1, "First Comment", text, UTF8Text, refs)
	if res := msg.ValidateBasic(); res == nil {
