...
func newContextAndKeeper(chainid string) (sdk.Context, *Keeper) {
	...
	k := keepers.NewKeeper(key,
		...
		&mocAssetStatusKeeper{assets: map[string]bool{"usdt": true, "btc": true, "mypc": true}},
...
func testGenesis(t *testing.T, ctx sdk.Context, keeper *Keeper) {
	...
	gns = NewGenesisState(map[string]uint64{"mypc": 100})
	InitGenesis(ctx, *keeper, gns)
	err := gns.Validate()
	require.Equal(t, nil, err)
	gns = ExportGenesis(ctx, *keeper)
	require.Equal(t, "map[mypc:100]", fmt.Sprintf("%v", gns.CommentCount))
}
...
func Test1(t *testing.T) {
	...
	require.Equal(t, uint64(100), keeper.GetCommentCount(ctx, "mypc"))
	...
	s = "http://google.com"
	refs := getRefs()
	msg := types.NewMsgCommentToken(simpleAddr("00003"), "mypc", 1, "First Comment", s, types.HTTP, refs)

	res = msgHandler(ctx, *msg)
	require.Equal(t, `Add 1mypc to comPool`, logStrList[0])
	require.Equal(t, uint64(100), keeper.GetCommentCount(ctx, "mypc"))
	...
	res = msgHandler(ctx, *msg)
	s = `Add 1mypc to comPool`
	require.Equal(t, s, logStrList[0])
	s = `Send 10usdt from mypos1qy352eufqy352eufqy352eufqy35qqqrnvpntj to mypos1qy352eufqy352eufqy352eufqy35qqqrnvpntj`
	require.Equal(t, s, logStrList[1])
	//s = `Msg(token_comment): {"id":100,"sender":"mypos1qy352eufqy352eufqy352eufqy35qqqrnvpntj","token":"mypc","donation":1,"title":"First Comment","content":"http://google.com","content_type":2,"references":[{"id":900,"reward_target":"mypos1qy352eufqy352eufqy352eufqy35qqqzw64xkq","reward_token":"mypc","reward_amount":0,"attitudes":[50,59]},{"id":901,"reward_target":"mypos1qy352eufqy352eufqy352eufqy35qqqrnvpntj","reward_token":"usdt","reward_amount":10,"attitudes":[52,59]}]}`
	//require.Equal(t, s, logStrList[2])
	require.Equal(t, uint64(101), keeper.GetCommentCount(ctx, "mypc"))
	...
	res = msgHandler(ctx, *msg)
	s = `{"codespace":"comment","code":911,"message":"No such account: mypos1qy352eufqy352eufqy352eufqy35qqq8uxr4yg"}`
	require.Equal(t, s, res.Log)
	require.Equal(t, false, res.IsOK())
}
