...
type GetFilePathAndFileInmyposchainFromDirCb func(string, int) (filePath string, fileInmyposchain int, err error)

type dirMsgWriter struct {
	io.WriteCloser
	haveWriteSize int
	fileInmyposchain     int
	dir           string
	timeNewFile   func(k, v []byte) bool
}

func NewDirMsgWriter(dir string, cb GetFilePathAndFileInmyposchainFromDirCb) (MsgWriter, error) {
	filePath, fileInmyposchain, err := cb(dir, MaxFileSize)
	if err != nil {
	...
	diw := &dirMsgWriter{
		WriteCloser:   file,
		fileInmyposchain:     fileInmyposchain,
		dir:           dir,
...
func (w *dirMsgWriter) WriteKV(k, v []byte) error {
	if w.timeNewFile(k, v) {
		...
		file, err := openFile(GetFileName(w.dir, w.fileInmyposchain+1))
		if err != nil {
			return err
		}
		w.WriteCloser = file
