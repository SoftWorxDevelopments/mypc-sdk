...
func TestGetFilePathAndFileInmyposchainFromDir(t *testing.T) {
	dirPath := "tmp"
	filePath, fileInmyposchain, err := GetFilePathAndFileInmyposchainFromDir(dirPath, 10)
	require.Nil(t, err)
	require.Equal(t, "tmp/backup-0", filePath)
	require.Equal(t, 0, fileInmyposchain)
	...
	require.Equal(t, 3, fileInmyposchain)
	...
	require.Equal(t, 4, fileInmyposchain)
